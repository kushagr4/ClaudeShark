...
# V2.4b: with more than this much clock, plan for fewer moves to go, so the
# surplus every rated game left unspent is used early. At or below the
# threshold the allocation is exactly the shipped one, so the equilibrium
# clock in a long game, the hard deadline, the reserve and the panic path are
# unchanged.
SURPLUS_CLOCK_MS = 60_000.0
SURPLUS_MOVES_TO_GO = 16
...
class TimeManager:
    ...
    def begin(self, time_left_ms: int, phase: int) -> None:
        ...
        if time_left_ms > SURPLUS_CLOCK_MS:
            moves_to_go = min(moves_to_go, SURPLUS_MOVES_TO_GO)
