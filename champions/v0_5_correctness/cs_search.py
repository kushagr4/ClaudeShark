...
def _flag(name: str, default: bool) -> bool:
    """Read an experiment flag from the environment.
...
# Allow transposition-table score cutoffs at PV nodes. On by default because
# that is what v0.2 shipped; turning it off restricts cutoffs to non-PV nodes.
TT_CUTOFF_AT_PV = _flag("CS_TT_PV_CUTOFF", True)
...
class Searcher:
    ...
    def _negamax(
        ...
        #
        # Checkmate outranks the fifty-move rule: a mate delivered on the
        # hundredth halfmove is a win, not a draw. Falling through when in check
        # lets the normal search decide, and is_check() is only paid on the
        # vanishingly rare nodes where the counter is actually exhausted.
        if board.halfmove_clock >= 100 and not board.is_check():
            return DRAW_SCORE
        ...
        if entry is not None:
            ...
            # is a different matter: it may come from a reduced-depth or
            # null-window search, so cutting on it at a PV node can truncate the
            # principal variation with a score that was never proven in this
            # window. TT_CUTOFF_AT_PV controls whether we do it anyway.
            if entry_depth >= depth and (TT_CUTOFF_AT_PV or beta - alpha == 1):
        ...
        moves = order_moves(
            board, moves, tt_move, ply, self.heuristics,
            _see_losing if USE_SEE_ORDER else None,
        )
    ...
    def _quiescence(
        ...
        # Rule draws, which quiescence previously ignored entirely. The
        # fifty-move test is a single integer comparison and the material test
        # exits on its first branch in any position with a pawn, rook or queen,
        # so both are affordable here. Checkmate outranks the fifty-move rule,
        # hence the in_check guard.
        if board.halfmove_clock >= 100 and not in_check:
            return DRAW_SCORE
        if is_material_draw(board):
            return DRAW_SCORE
